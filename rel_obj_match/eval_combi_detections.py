...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        default="/is/rg/ev/scratch/celich/data/Hypersim/Hypersim_SuperGlue-keypoints_ICRA23/processed_results_EfficientDet-D7",
    ...
    parser.add_argument("--detector", default="EfficientDet-D7")
    parser.add_argument(
        "--message",
        default="",
...
def main(model, cnfg, data, data_org, eval_dir):
    ...
    ops = {
        ...
        "iterator_org": iterator_test_org,
    }
...
def eval_model(net, data, ops, eval_dir):
    ...
    for batch_id in range(num_batches):
        ...
        input_org_batch = next(ops["iterator_org"])
        input_batch = get_input(input_batch)  # , data.model_type)
        input_org_batch = net.get_input(input_org_batch)
        ...
        if (
            ...
            # get superglue keypoint matching results
            ...
            # vote_path = os.path.join(KEYPONT_MATCHING_RES_DIR, scene_names[0], 'keypoint-votes' + pair_name)
        ...
        for cm in COMBI_MODI:
            ...
            tools_eval.eval_quantitative_single_batch_affinity_detections(
                {**input_batch, **output},
                input_org_batch,
                eval_dict_combi[cm],
    ...
    for cm in COMBI_MODI:
        eval_dict_combi_updated[cm] = tools_eval.finalize_eval_scores_detections(
            eval_dict_combi[cm]
        )  # --> DIFF(repeat)
        if eval_dict_combi_updated[cm]["score_match_gt-all_f1"] > best_f1score:
            best_f1score = eval_dict_combi_updated[cm]["score_match_gt-all_f1"]
...
if __name__ == "__main__":
    ...
    if os.path.exists(os.path.join(LOG_DIR, "ckpts", "checkpoint")):
        ...
        cnfg["data"]["use_pretrained"] = False
        cnfg["model"]["backbone"] = "ResNet34"
    else:
    ...
    dataset.set_model_type(model_ext_name)
    dataset_org = data_provider_module.get_dataset(
    ...
    main(model, cnfg, dataset, dataset_org, eval_dir)
